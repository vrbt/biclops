...
import com.github.vrbt.biclops.decoders.implementation.BigIntegerDecoderBuilder
...
class BigIntegerTest extends Specification {
    ...
    def 'decode a whole BigInteger from the buffer'() {
        ...
        def decoder = new BigIntegerDecoderBuilder().valueLengthInBits intValue.toByteArray().size() * 8 build()
    ...
    def 'decode a whole BigInteger from the buffer - little endian'() {
        ...
        def decoder = new BigIntegerDecoderBuilder().byteOrder LITTLE_ENDIAN build()
    ...
    def 'decode a whole variable length BigInteger from the buffer'() {
        given:
        def ByteBuffer buffer = ByteBuffer.wrap intValue
        def decoder = new BigIntegerDecoderBuilder().valueLengthInBits bitLength build()
    ...
    def 'decode a 4-bit part of a long from the buffer'() {
        ...
        def decoder = new BigIntegerDecoderBuilder().valueLengthInBits 4 build()
    ...
    def 'decode a 60-bit part of a long from the buffer'() {
        given:
        def ByteBuffer buffer = ByteBuffer.wrap Longs.toByteArray(intValue)
        def decoder = new BigIntegerDecoderBuilder().valueLengthInBits 60 build()
    ...
    def 'decode a various length part of a long from the buffer and shift the buffer'() {
        given:
        def ByteBuffer buffer = ByteBuffer.wrap Longs.toByteArray(intValue)
        def decoder = new BigIntegerDecoderBuilder().valueLengthInBits bitsUsed build()
